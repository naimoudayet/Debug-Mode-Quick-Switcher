...
## [16.0.1.3.1] - 2026-06-01

### Changed
- Updated the Access Pass cross-promo card price from $79 to $99 to match its current price.

## [16.0.1.3.0] - 2026-05-14

### Added
- **Backport from 17.0.1.3.0** — full feature parity: 9 languages with native translations, unified Selection labels matching the navbar dropdown, App Store storefront, full Python test coverage (9 tests). The `?debug=` anchor-navigation mechanism (already canonical in v16's own Settings > Developer Tools) is shared with the 17.0 implementation.

### Changed
- **v16-compatible i18n calls** — at module top level (DEBUG_MODES labels, hotkey hint) `_t` is replaced with `_lt` from `@web/core/l10n/translation`. v16's `_t` resolves eagerly at call time, so a top-level `_t("Off")` would freeze the English source string before translations finish loading; `_lt` returns a `LazyTranslatedString` that resolves on stringification (the same behavior `_t` got in v18+). Parametrized strings in getters wrapped with `sprintf` from `@web/core/utils/strings` since v16's `_t` takes only one argument.
- **Dropped the FormController save patch** — `FormController.prototype.onRecordSaved` does not exist in v16 (added in v17). Implementing it via `save()` snapshot/compare would be brittle against v16's record-data shape. Trade-off: when changing the default mode from My Profile, the redirect is deferred to the next navigation instead of firing instantly. The forward path (navbar dropdown → mode switch + pref write) is unchanged.
- Module version: `16.0.1.3.0`. Port: `13416`.
